...
namespace BestiaryMod
{
    ...
    /// </summary>
    public class BestiaryModCSVParser
    {
        ...
        /// <param name="skipModAssests">If TRUE, skips mod instance and directly checks StreamingAssets.</param>
        /// <returns>KeyValuePair for each row if successful, otherwise null.</returns>
        public static List<KeyValuePair<string, string>> Load(string filename, bool skipModAssests = false)
        {
            string csvText = null;

            // Seek mods first then try loading from StreamingAssets/Text
            TextAsset csvTextAsset = null;
            if (!skipModAssests && ModManager.Instance != null && ModManager.Instance.TryGetAsset(filename, false, out csvTextAsset))
            {
        ...
        public static Dictionary<string, string> LoadDictionary(string filename, bool skipModAssests = false)
        {
            Dictionary<string, string> dict = new Dictionary<string, string>();

            var rows = Load(filename, skipModAssests);
